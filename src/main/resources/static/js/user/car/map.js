...
function drawRegion(map, regionList) {
    ...
    for (var i = 0; i < regionList.length; i++) {
        ...
        if (regionList[i].enName in regionCountMap) {
            carCount = regionCountMap[regionList[i].enName];
        }
        ...
        (function (textContent) { // 闭包传参
            boundary.get(city.cnName + regionList[i].cnName, function(rs) { // 获取行政区域
    ...
    if (!customLayer) {
        ...
        customLayer.addEventListener('onhotspotclick', carTip); // 单击图层事件
    }
    ...
    function carTip(e) {
        ...
        var content =  '<p style="width:280px;margin: 0; line-height: 20px;">地址：' +
            customPoi.address + '<br/>价格：' + contentPoi.price + '元<br/>品牌：'
            + contentPoi.brand + '-' + contentPoi.series + '</p>';
        var opts = {
            width: 200, // 信息窗口宽度
            height: 100, // 信息窗口高度
    ...
    layui.use('flow', function () {
        ...
        flow.load({
            ...
            scrollElem: '#car-flow',
                ...
                $.get('/user/car/map/cars?cityEnName=' + cityName + '&start=' + start + '&size=3' + target,
                    function (res) {
                        if (res.code !== 200) {
                            lis.push('<li>数据加载错误</li>');
                        } else {
                            layui.each(res.data, function (index, car) {
                                var tags = '';
                                for (var i = 0; i < car.tags.length; i++) {
                                    tags += '<span class="item-tag-color_2 item-extra">' + car.tags[i].name + '</span>';
                                }
                                var li = '<li class="list-item"><a href="/user/car/detail/' + car.id + '" target="_blank"'
                                    + ' title="' + car.title + '"data-community="1111027382235"> <div class="item-aside">'
                                    + '<img src="' + car.cover + '?imageView2/1/w/116/h/116"><div class="item-btm">'
                                    + '<span class="item-img-icon"><i class="i-icon-arrow"></i><i class="i-icon-dot"></i>'
                                    + '</span>&nbsp;&nbsp;</div></div><div class="item-main"><p class="item-tle">'
                                    + car.title + '</p><p class="item-des"> <span>' + car.brand + '-' + car.series
                                    + '</span><span>' + car.mileage + 'km</span> <span>' + car.seats + '座</span>'
                                    + '<span class="item-side">' + car.price + '<span>元</span></span></p>'
                                    + '<p class="item-community"><span class="item-replace-com">' + car.description + '</span>'
                                    + '<span class="item-exact-com">' + car.description + '</span></p><p class="item-tag-wrap">'
                                    + tags + '</p></div></a></li>';
