var fastSerachCarTagAss;

$(function(){
    fastSerachCarTagAss = new FastSearchCarTagAss().init();
});

var FastSearchCarTagAss = (function(){
    ...
    var _batchDelete = function(){
        ...
        //提交请求
        layer.confirm('标签分派信息删除须谨慎，确认要删除吗？', function (index) {
            $.ajax({
                url: '/car/tag/add/batch',
                type: 'POST',
    ...
    var _edit = function(){
        var title = '编辑标签分派';
        var url = '/admin/car/tag/ass/edit?id=' + $(this).attr("primaryKey");
        var width = '900';
        var height;
    ...
    var _delete = function(){
        ...
        layer.confirm('标签分派信息删除须谨慎，确认要删除吗？', function (index) {
            $.ajax({
                url: '/car/tag/ass/' + $(obj).attr("primaryKey"),
                type: 'DELETE',
                dataType: 'json',
                success: function (data) {
        ...
        if(data && (data instanceof Array)){
            ...
            $.each(data, function(index, item){
                _html += _template.replace("{{id}}", item.id)
                    .replace("{{id}}", item.id)
                    .replace("{{id}}", item.id)
                    .replace("{{carTitle}}", item.carTitle)
                    .replace("{{carTagName}}", item.carTagName);
            });
     ...
     * 内部对象
     */
    var _fastSerachCarTagAss = function () {};

    /**
     * 初始化代码
     */
    _fastSerachCarTagAss.prototype.init = function(){
        _init();
        _loadData();
    };

    return _fastSerachCarTagAss;
})();